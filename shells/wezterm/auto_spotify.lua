...
local function is_any_sound_playing()
  ...
  if not players_handle then return false end
  for player in players_handle:lines() do
    local handle = io.popen("playerctl -p " .. player .. " status")
    if not handle then return false end
    local result = handle:read("*a")
    handle:close()
    if result:match("Playing") then return true end
  end
end

local function play_spotify()
  os.execute(
    "notify-send --transient --app-name='Wezterm Auto Spotify' 'Spotify' 'Playing Spotify'"
  )
